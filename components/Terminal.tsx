...
import { THEME_KEYS, applyTheme } from '@/lib/themes';
...
const commands: Record<string, () => string[]> = {
  ...
  skills: () => [
    'Technical Skills:',
    ...skills.coding.map((s) => `  ${s.name}: ${s.level}%`),
    '',
    'Knowledge:',
    ...skills.knowledge.map((k) => `  - ${k}`),
  ],
  projects: () => [
    'Featured Projects:',
    ...projects.slice(0, 6).map((p, i) => `  ${i + 1}. ${p.title} - ${p.link}`),
    '',
  ...
  themes: () => [
    'Available themes:',
    ...THEME_KEYS.map((theme, i) => `  ${theme}${i === 0 ? '  (light)' : ''}`),
    '',
    'Use "theme <name>" to change theme.',
  ],
  date: () => [new Date().toString()],
  whoami: () => [`visitor@${site.brand} ~ exploring awesome projects`],
  ls: () => ['about/', 'projects/', 'contact/', 'settings/', 'README.md'],
